...
class NetflixReport(FPDF):
    ...
    def title_page(self):
        ...
        self.ln(8)
        self.cell(0, 10, 'Content-Based Recommendations | Classification | Clustering | Forecasting | BI', align='C')
    ...
    def add_image_half(self, path1, path2, w=90):
        if Path(path1).exists():
            x1 = 15
            self.image(path1, x=x1, w=w)
        if Path(path2).exists():
            x2 = 110
            self.image(path2, x=x2, w=w)
        self.ln(4)
...
def build_report():
    ...
    sample_rows = [
        ['Stranger Things', 'TV Show', 'TV Horror, Mysteries, Sci-Fi', '0.809'],
        ['The Crown', 'TV Show', 'British TV, Dramas', '0.849'],
        ['Black Mirror', 'TV Show', 'British TV, Dramas', '0.727'],
        ['Narcos', 'TV Show', 'Crime, Dramas, Action', '0.860'],
        ['3 Idiots', 'Movie', 'Comedies, Dramas, International', '1.000'],
    ]
    pdf.add_table(['Query Title', 'Top Match', 'Match Genre', 'Score'], sample_rows, [40, 50, 60, 25])
    ...
    pdf.body_text(
        ...
        'based on its content attributes including genres, countries, ratings, durations, release '
        'year, and other engineered features.'
    )

    pdf.sub_title('Workflow')
    pdf.bullet('Step 1: Selected 9 numerical features (release_year, duration_min, duration_seasons, etc.) and 6 categorical features (type, primary_country, primary_genre, rating_category)')
    ...
    pdf.bullet('Step 4: Evaluated on held-out test set (20% of data) using accuracy, precision, recall, F1')
    ...
    pdf.add_table(['Model', 'CV F1', 'Val F1', 'Test F1', 'Train Time'], model_rows, [40, 25, 25, 25, 25])
    ...
    cluster_rows = [
        ['K-Means', '4', '0.42', '0.78', '1450'],
        ['Hierarchical', '4', '0.40', '0.80', '0'],
        ['GMM', '4', '0.41', '0.79', '0'],
        ['DBSCAN', 'N/A', 'N/A', 'N/A', '1200'],
    ]
    pdf.add_table(['Algorithm', 'Clusters', 'Silhouette', 'Davies-Bouldin', 'Noise'], cluster_rows, [35, 25, 30, 35, 30])

    pdf.sub_title('Identified Clusters')
    pdf.check_page_break(50)
    pdf.body_text(
        'Cluster 0 (5.7%): Classic Movies — older films (avg 1986) dominated by Action & Adventure and '
        'Comedies. Mostly movies with very few TV shows.'
    )
    pdf.body_text(
        'Cluster 1 (27.7%): International TV Dramas — dominated by non-US productions, Crime TV Shows, '
        'and Kids TV. Recent content (avg 2017). Largest TV Show cluster.'
    )
    pdf.body_text(
        'Cluster 2 (38.2%): Mainstream Movies — diverse movies including Dramas, Comedies, and Action. '
        'Recent content (avg 2016). Largest single cluster.'
    )
    pdf.body_text(
        'Cluster 3 (28.4%): Documentaries & Dramas — mix of documentaries and TV shows alongside '
    ...
    pdf.body_text(
        ...
        'Prophet struggles with the limited training data (only ~7 years), producing the highest errors. '
    ...
    pdf.bullet('Director Network: 4,527 unique directors; 29.4% of titles have "Not Given" director')
    ...
    pdf.body_text('6. Collaborative productions exist (7% multi-director titles) — leverage this for cross-market content')
    ...
    pdf.add_table(['Task', 'Description', 'Best Model', 'Key Metric', 'Algorithm'], summary_rows, [18, 38, 42, 42, 22])
    ...
    pdf.bullet('Complete Jupyter notebook (48 cells) for interactive exploration')
    ...
    # Save
    report_path = PROJECT_ROOT / 'Netflix_ML_Project_Report.pdf'
